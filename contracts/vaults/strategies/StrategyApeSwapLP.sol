...
import "../interfaces/ape/IMiniChefV2.sol";
import "../interfaces/ape/IRewarder.sol";
...
contract StrategyApeSwapLP is StratManager, FeeManager {
    ...
    constructor(
        ...
        address _unirouter,
        address _strategist,
        address[] memory _keeperAndFee, // combining keeper & fee to avoid stack too deep error
        address[] memory _outputToNativeRoute,
        address[] memory _rewardToOutputRoute,
        address[] memory _outputToLp0Route,
        address[] memory _outputToLp1Route
    ) StratManager(_keeperAndFee[0], _strategist, _unirouter, _vault, _keeperAndFee[1]) {
        want = _want;
        poolId = _poolId;
        chef = _chef;

        require(_keeperAndFee.length == 2, "_keeperAndFee needs exactly two items");
        ...
        require(_rewardToOutputRoute.length >= 2, "invalid length on _rewardToOutputRoute");
        reward = _rewardToOutputRoute[0];
        require(_rewardToOutputRoute[_rewardToOutputRoute.length - 1] == output, "_rewardToOutputRoute[last] != output");
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        if (callFeeRecipient != tx.origin) {
            IERC20(native).safeTransfer(callFeeRecipient, callFeeAmount);
        } else {
            IERC20(native).safeTransfer(tx.origin, callFeeAmount);
        }

        uint256 feeAmount = nativeBal.mul(barbershopFee).div(MAX_FEE);
    ...
    function rewardsAvailable() public view returns (uint256) {
        return IMiniChefV2(chef).pendingBanana(poolId, address(this));
    }
